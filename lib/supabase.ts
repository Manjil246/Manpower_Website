            ...
            setTestimonials((prev) =>
              prev.map((testimonial) =>
                testimonial.id === payload.new.id
                  ? (payload.new as Testimonial)
                  : testimonial
              )
            );
          } else if (payload.eventType === "DELETE") {
            setTestimonials((prev) =>
              prev.filter((testimonial) => testimonial.id !== payload.old.id)
            );
          }
        }
      )
...
export const useRealtimeStats = () => {
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "company_stats" },
        (payload) => {
...
export const submitContactInquiry = async (
  inquiryData: Omit<ContactInquiry, "id" | "status" | "created_at">
) => {
  const { data, error } = await supabase
